...
def buy():
    ...
    if request.method == "POST":
        ...
        if user_cash[0]["cash"] < total_cost:
            ...
            # Check if 'name' key exists in stock_quote dictionary
            stock_name = stock_quote.get('name', 'Unknown')

            db.execute("INSERT INTO trades (id, symbol, name, shares, price) VALUES(?, ?, ?, ?, ?)",
            session["user_id"], stock_quote['symbol'], stock_name, int(shares_nbr), stock_quote['price'])
